#!/usr/bin/env bash
# Compare similar pretrained DiffSplat backbones on the same prompt subset.
# Requires checkpoints for each model in MODELS.

#SBATCH --job-name=diffsplat-models
...
#SBATCH --output=/fs/nexus-scratch/gnanesh/DiffSplat/logs/slurm/model_compare_%j.log
...
PROMPT_FILE="experiments/prompts/t3bench_model_compare_${PROMPT_COUNT}.txt"
...
python scripts/collect_ablation_results.py \
  --root out_ablation \
  --json results/model_compare_summary.json \
  --csv results/model_compare_summary.csv
